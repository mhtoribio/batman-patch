...
enum batadv_nl_attrs {
	 ...
	 *  used by B.A.T.M.A.N. V when estimating the link throughput. When
	 *  used together with @BATADV_CMD_SET_HARDIF it applies to the whole
	 *  interface. When used together with
	 *  @BATADV_CMD_SET_NEIGH_THROUGHPUT_OVERRIDE it applies to one
	 *  neighbour on the selected interface. If the value is set to 0 then
	 *  the configured override is removed and batman-adv will try to
...
enum batadv_nl_commands {
	...
	/**
	 * @BATADV_CMD_SET_NEIGH_THROUGHPUT_OVERRIDE: Set throughput override
	 *  for one neighbour of one hard-interface in the current mesh(if)
	 */
	BATADV_CMD_SET_NEIGH_THROUGHPUT_OVERRIDE,

	/**
	 * @BATADV_CMD_GET_NEIGH_THROUGHPUT_OVERRIDES: Query list of neighbour
