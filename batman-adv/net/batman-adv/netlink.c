...
/**
 * batadv_netlink_neigh_tp_override_fill() - Fill message with neighbour
 *  throughput override attributes
 * @msg: Netlink message to dump into
 * @bat_priv: the bat priv with all the mesh interface information
 * @hard_iface: hard interface owning the override entry
 * @addr: mac address of the neighbour
 * @throughput: configured throughput override
 * @cmd: type of message to generate
 * @portid: Port making netlink request
 * @seq: sequence number for message
 * @flags: Additional flags for message
 ...
 */
static int
batadv_netlink_neigh_tp_override_fill(struct sk_buff *msg,
				      struct batadv_priv *bat_priv,
				      struct batadv_hard_iface *hard_iface,
				      const u8 *addr, u32 throughput,
				      enum batadv_nl_commands cmd,
				      u32 portid, u32 seq, int flags,
				      struct netlink_callback *cb)
{
	...
	if (nla_put_u32(msg, BATADV_ATTR_HARD_IFINDEX,
	    ...
	    nla_put_string(msg, BATADV_ATTR_HARD_IFNAME,
			   net_dev->name) ||
	    nla_put(msg, BATADV_ATTR_NEIGH_ADDRESS, ETH_ALEN, addr) ||
	    nla_put_u32(msg, BATADV_ATTR_THROUGHPUT_OVERRIDE, throughput))
		goto nla_put_failure;
...
/**
 * batadv_netlink_set_neigh_throughput_override() - Set neighbour throughput
 *  override
 * @skb: Netlink message with request data
 * @info: receiver information
 *
 ...
 */
static int
batadv_netlink_set_neigh_throughput_override(struct sk_buff *skb,
					     struct genl_info *info)
{
#ifdef CONFIG_BATMAN_ADV_BATMAN_V
	struct batadv_hard_iface *hard_iface = info->user_ptr[1];
	...
	if (!info->attrs[BATADV_ATTR_NEIGH_ADDRESS] ||
	    !info->attrs[BATADV_ATTR_THROUGHPUT_OVERRIDE])
		return -EINVAL;

	addr = nla_data(info->attrs[BATADV_ATTR_NEIGH_ADDRESS]);
	if (!is_valid_ether_addr(addr))
		return -EINVAL;

	throughput = nla_get_u32(info->attrs[BATADV_ATTR_THROUGHPUT_OVERRIDE]);

	return batadv_v_elp_throughput_override_set(hard_iface, addr,
						    throughput);
#else
	return -EOPNOTSUPP;
#endif
...
/**
 * batadv_netlink_dump_neigh_tp_overrides_hardif() - Dump neighbour throughput
 *  overrides of a hard interface into messages
 * @msg: Netlink message to dump into
 * @portid: Port making netlink request
 ...
 */
static int
batadv_netlink_dump_neigh_tp_overrides_hardif(struct sk_buff *msg, u32 portid,
					      u32 seq,
					      struct batadv_priv *bat_priv,
					      struct batadv_hard_iface *hard_iface,
					      int *idx_s,
					      struct netlink_callback *cb)
{
	struct batadv_neigh_throughput_override *entry;
	int idx = 0;

	spin_lock_bh(&hard_iface->tp_override.lock);

	hlist_for_each_entry(entry, &hard_iface->tp_override.entries, list) {
		if (idx++ < *idx_s)
			continue;

		if (batadv_netlink_neigh_tp_override_fill(msg, bat_priv,
							  hard_iface,
							  entry->addr,
							  entry->throughput,
							  BATADV_CMD_GET_NEIGH_THROUGHPUT_OVERRIDES,
							  portid, seq,
							  NLM_F_MULTI, cb)) {
			*idx_s = idx - 1;
			spin_unlock_bh(&hard_iface->tp_override.lock);
			return -EMSGSIZE;
		}
	}
...
/**
 * batadv_netlink_dump_neigh_throughput_overrides() - Dump neighbour
 *  throughput overrides into messages
 * @msg: Netlink message to dump into
 ...
 */
static int
batadv_netlink_dump_neigh_throughput_overrides(struct sk_buff *msg,
					       struct netlink_callback *cb)
{
#ifdef CONFIG_BATMAN_ADV_BATMAN_V
	struct batadv_hard_iface *single_hard_iface = NULL;
	...
	single_hard_iface = batadv_netlink_get_hardif(bat_priv, cb);
	if (IS_ERR(single_hard_iface)) {
		ret = PTR_ERR(single_hard_iface);
		if (ret == -ENONET)
			single_hard_iface = NULL;
		else
			goto out_put_meshif;
	}
	...
	if (single_hard_iface) {
		if (i_hardif_s == 0 &&
		    batadv_netlink_dump_neigh_tp_overrides_hardif(msg, portid,
								  cb->nlh->nlmsg_seq,
								  bat_priv,
								  single_hard_iface,
								  &idx, cb) == 0)
		...
		netdev_for_each_lower_private(mesh_iface, hard_iface, iter) {
			...
			if (batadv_netlink_dump_neigh_tp_overrides_hardif(msg,
									  portid,
									  cb->nlh->nlmsg_seq,
									  bat_priv,
									  hard_iface,
	...
	if (single_hard_iface)
		batadv_hardif_put(single_hard_iface);
	dev_put(mesh_iface);
...
static const struct genl_small_ops batadv_netlink_ops[] = {
	...
	{
		.cmd = BATADV_CMD_SET_NEIGH_THROUGHPUT_OVERRIDE,
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.flags = GENL_UNS_ADMIN_PERM,
		.doit = batadv_netlink_set_neigh_throughput_override,
		.internal_flags = BATADV_FLAG_NEED_MESH |
				  BATADV_FLAG_NEED_HARDIF,
	},
	{
		.cmd = BATADV_CMD_GET_NEIGH_THROUGHPUT_OVERRIDES,
		.validate = GENL_DONT_VALIDATE_STRICT | GENL_DONT_VALIDATE_DUMP,
		.flags = GENL_UNS_ADMIN_PERM,
		.dumpit = batadv_netlink_dump_neigh_throughput_overrides,
		.internal_flags = BATADV_FLAG_NEED_MESH,
	},
};
...
struct genl_family batadv_netlink_family __ro_after_init = {
	...
	.resv_start_op = BATADV_CMD_GET_NEIGH_THROUGHPUT_OVERRIDES + 1,
